...
bool S9xInitMemory(void)
{
   ...
   Memory.MapInfo = (SMapInfo*)snes_calloc(MEMMAP_NUM_BLOCKS, sizeof(SMapInfo));

   IPPU.ScreenColors = (uint16_t *)snes_calloc(256 * 9, sizeof(uint16_t));
   IPPU.DirectColors = IPPU.ScreenColors + 256;
   IPPU.TileCache = (uint8_t*) snes_calloc(MAX_2BIT_TILES, 128);
   IPPU.TileCached = (uint8_t*) snes_calloc(MAX_2BIT_TILES, 1);
...
{
   snes_free(Memory.RAM);
   Memory.RAM = NULL;

   snes_free(Memory.SRAM);
   Memory.SRAM = NULL;

   snes_free(Memory.VRAM);
   Memory.VRAM = NULL;

   if (Memory.ROM)
      snes_free(Memory.ROM - Memory.ROM_Offset);
   Memory.ROM = NULL;
   Memory.ROM_Offset = 0;
   Memory.ROM_AllocSize = 0;

   snes_free(Memory.FillRAM);
   Memory.FillRAM = NULL;

   snes_free(Memory.Map);
   Memory.Map = NULL;

   snes_free(Memory.MapInfo);
   Memory.MapInfo = NULL;

   snes_free(IPPU.ScreenColors);
   IPPU.ScreenColors = NULL;

   snes_free(IPPU.TileCached);
   IPPU.TileCached = NULL;

   snes_free(IPPU.TileCache);
   IPPU.TileCache = NULL;

   snes_free(bytes0x2000);
   bytes0x2000 = NULL;
}
