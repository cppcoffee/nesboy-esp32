...
\
   ...
   { \
      apu.rectangle[ch].accum += (apu.rectangle[ch].freq + 1) << APU_CYCLE_FRAC_BITS; \
...
static inline int apu_triangle(void)
{
   ...
   {
      apu.triangle.accum += apu.triangle.freq << APU_CYCLE_FRAC_BITS;
...
static inline int apu_noise(void)
{
   ...
   */
   while (apu.noise.accum < 0)
   {
      ...
      apu.noise.accum += apu.noise.freq << APU_CYCLE_FRAC_BITS;
   }
...
static inline int apu_dmc(void)
{
   ...
   if (apu.dmc.dma_length)
   {
      ...
      {
         apu.dmc.accum += apu.dmc.freq << APU_CYCLE_FRAC_BITS;
...
void apu_write(uint32 address, uint8 value)
{
   ...
   case APU_WRC3:
      ...
      ** the other triregs
      */
      apu.triangle.write_latency = (228 << APU_CYCLE_FRAC_BITS) / apu.cycle_rate;
...
static void apu_render_samples(int num_samples)
{
   if (num_samples <= 0)
      return;

   int channels = apu.stereo ? 2 : 1;
   apu_process(apu.buffer + apu.samples_generated * channels, num_samples, apu.stereo);
   apu.samples_generated += num_samples;
}

void apu_renderline(int scanlines_per_frame)
{
   apu.sample_accum += apu.samples_per_frame;
   int num_samples = apu.sample_accum / scanlines_per_frame;
   apu.sample_accum -= num_samples * scanlines_per_frame;
   apu_render_samples(num_samples);
}

void apu_emulate(void)
{
   apu_render_samples(apu.samples_per_frame - apu.samples_generated);
   apu.samples_generated = 0;
   apu.sample_accum = 0;
}
...
void apu_reset(void)
{
   ...
   apu.samples_generated = 0;
   apu.sample_accum = 0;
   apu.cycle_rate = (int)(((int64)nes->cpu_clock << APU_CYCLE_FRAC_BITS) / apu.sample_rate);
